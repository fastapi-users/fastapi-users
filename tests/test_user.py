...
class TestCreateUser:
    ...
    async def test_existing_user(self, email, create_user):
        user = UserCreate(email=email, password="guinevere", username="king.arthur")
    ...
    async def test_regular_user(self, email, create_user):
        user = UserCreate(email=email, password="guinevere", username="lancelot")
    ...
    async def test_superuser(self, create_user, safe, result):
        user = UserCreate(
            email="lancelot@camelot.b", password="guinevere", is_superuser=True, username="lancelot"
        )
    ...
    async def test_is_active(self, create_user, safe, result):
        user = UserCreate(
            email="lancelot@camelot.b", password="guinevere", is_active=False, username="lancelot"
        )
        created_user = await create_user(user, safe)
        assert type(created_user) == UserDB
